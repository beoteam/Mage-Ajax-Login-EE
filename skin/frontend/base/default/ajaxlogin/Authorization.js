if ( window.Prototype ) {
    ...
    AjaxLogin.Authorization.prototype = {
        ...
        setMessage: function(message, type, delay) {
            ...
            var __messageNodeText = document.createElement('SPAN');
            var __messageNodeTextContainer = document.createElement('LI');
            var __messageNodeContainer = document.createElement('UL');
            var __messageNode = document.createElement('LI');

            __messageNodeTextContainer.appendChild(__messageNodeText);
            __messageNodeContainer.appendChild(__messageNodeTextContainer);
            __messageNode.appendChild(__messageNodeContainer);
            ...
            __messageNodeTextContainer.innerHTML = message;
        ...
        __makeResponseReaction: function(response) {
            if ( typeof response != 'undefined' ) {
                ...
                if ( typeof response.errorMessage != 'undefined' ) {
                    ...
                    if (typeof response.captcha != 'undefined' && $('captcha-input-box-' + response.captcha)) {
                        $('captcha-input-box-' + response.captcha).show();
                    }
                    if (typeof response.form_id != 'undefined' && $(response.form_id)) {
                        $(response.form_id).captcha.refresh($(response.form_id));
                    }
                }
                if ( typeof response.successMessage != 'undefined' ) {
                    this.setMessage(response.successMessage, 1, 30);
                }
                if (typeof(AW_AjaxCartPro) != 'undefined' && typeof(AWACP_CONFIG) != 'undefined') {
        ...
        __resizeFrameObserver: function() {
             var me = this;
            Event.observe(window, "resize", function(){
                me.__prepareFramePosition();
            });
        },

        __prepareFramePosition: function() {
            //prepare popup position
            var currentFrame = [];
            ...
            var agreementPopup = $$('.alAgreementPopup .Canvas').first();
            if (agreementPopup) {
                currentFrame.push(agreementPopup);
            }

            currentFrame.each(function(element){
                var middleWindowTop = document.documentElement.offsetHeight/2;
                var middleFrameTop = element.getHeight()/2;
                var middleWindowLeft = document.documentElement.offsetWidth/2;
